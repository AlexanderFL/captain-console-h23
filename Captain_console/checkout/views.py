...
def base_context(id, context):
    ...
    context['product'] = Product.objects.raw(\
                         'SELECT * FROM store_product, store_orderproduct, store_order ' \
                         'WHERE store_product.id = store_orderproduct.product_id_id ' \
                         'AND store_orderproduct.order_id_id = store_order.id ' \
                         'AND store_order.user_id_id = s%;', [id])
    return context
